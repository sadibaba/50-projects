...
const BlogDetail = () => {
  ...
  const fetchBlog = async () => {
    try {
      ...
      if (!blogData) { setError('Blog post not found'); return; }
      ...
      setBlog({
        ...
        date: blogData.createdAt
          ? new Date(blogData.createdAt).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })
          : 'Unknown date',
  ...
  const handleLike = async () => {
    ...
    try {
      if (liked) {
        await unlikePost(id);
        setBlog(prev => ({ ...prev, likes: prev.likes?.filter(l => l !== user.id) || [], likesCount: prev.likesCount - 1 }));
      } else {
        await likePost(id);
        setBlog(prev => ({ ...prev, likes: [...(prev.likes || []), user.id], likesCount: prev.likesCount + 1 }));
      }
      setLiked(!liked);
    } catch (err) {
  ...
  const handleAddComment = async () => {
    ...
    try {
      const response = await addComment({ postId: id, content: newComment.trim() });
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-black flex items-center justify-center px-4">
        <div className="text-center">
          ...
          <p className="text-gray-400 mb-6 text-sm">{error || "This blog post doesn't exist."}</p>
        ...
        <article className="max-w-3xl mx-auto">
          ...
          <div className="mb-8">
            ...
            <div className="flex flex-wrap items-center justify-between gap-4 mb-8">
              <Link to={`/profile/${blog.authorName}`} className="flex items-center gap-3 hover:opacity-80 transition-opacity">
                <div className="w-10 h-10 rounded-full bg-gradient-to-r from-purple-500 to-pink-500 flex items-center justify-center flex-shrink-0">
                  <span className="text-white font-bold">{blog.authorName?.charAt(0).toUpperCase() || 'A'}</span>
                </div>
                <div>
                  <p className="text-white font-medium text-sm">{blog.authorName}</p>
                  <p className="text-gray-400 text-xs">{blog.date} · {blog.readTime} · {blog.views || 0} views</p>
                </div>
              </Link>
              ...
              >
                {likeLoading ? (
                  ...
                  <>
                    <svg className={`w-4 h-4 ${liked ? 'fill-pink-400' : ''}`} fill={liked ? 'currentColor' : 'none'} stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="border-t border-gray-800 pt-10">
            ...
            {!user ? (
              <div className="mb-8 p-6 bg-gray-800/50 rounded-xl text-center">
                <p className="text-gray-300 mb-4 text-sm">Please log in to leave a comment</p>
                <Link
                ...
                <div key={comment.id} className="bg-gray-800/50 rounded-xl p-5">
                  <div className="flex items-center gap-3 mb-3">
                    <div className="w-9 h-9 rounded-full bg-gradient-to-r from-purple-900 to-pink-900 flex items-center justify-center flex-shrink-0">
                      <span className="text-white text-sm font-medium">{comment.authorName?.charAt(0).toUpperCase() || 'A'}</span>
                    </div>
                    <div>
